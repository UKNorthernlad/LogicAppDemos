...
Param(
    ...
    [string] $ResourceGroupName = 'LogicAppsWorkshopDemos',
...
if ($ValidateOnly) {
    ...
    $output = New-AzResourceGroupDeployment -Name ((Get-ChildItem $TemplateFile).BaseName + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
